...
(def β (/ π 30))                        ; curvature of the rows
...
;; for right trackball
;; (def thumb-style "tr")                ; toggles between "default", "mini", and "cf", "tr" thumb cluster

;; for left
(def thumb-style "cf")                ; toggles between "default", "mini", and "cf", "tr" thumb cluster

(def nrows numrows)
(def ncols (+ numcols (if inner-column 1 0)))
...
(defn column-offset [column]
  (if inner-column
      (cond (<= column 1) [0 -2 0]
          (= column 3) [0 4 -2]     ; longest finger
          (= column 5) [0 -5 2]    ; original [0 -5.8 5.64]
          (= column 6) [0 -5.5 2.5]
          :else [0 0 0])
    (cond (= column 2) [0 4 -2]     ; longest finger
          (>= column 4) [0 -4 3]    ; original [0 -5.8 5.64]
          :else [0 0 0])))
...
(def caps
  (apply union
         (conj (for [column columns
               ...
               (->> (sa-cap (if (and pinky-15u (= column lastcol) (not= row lastrow)) 1.25 1))
                    (key-place column row)))
...
(if pinky-15u
  (do (def wide-post-tr (translate [(- (/ mount-width 1.3) post-adj)  (- (/ mount-height  2) post-adj) 0] web-post))
    (def wide-post-tl (translate [(+ (/ mount-width -1.3) post-adj) (- (/ mount-height  2) post-adj) 0] web-post))
    (def wide-post-bl (translate [(+ (/ mount-width -1.3) post-adj) (+ (/ mount-height -2) post-adj) 0] web-post))
    (def wide-post-br (translate [(- (/ mount-width 1.3) post-adj)  (+ (/ mount-height -2) post-adj) 0] web-post)))
...
(defn trthumb-tl-place [shape]
  (->> shape
       ...
       (translate thumborigin)
       (translate [-35 -16 -2]))) ; original 1.5u (translate [-32 -15 -2])))
(defn trthumb-mr-place [shape] ; center bottom
  (->> shape
       ...
       (translate thumborigin)
       (translate [-21 -37 -6])))
(defn trthumb-br-place [shape] ; bottom left
  (->> shape
       ...
       (translate thumborigin)
       (translate [-38 -47 -16])))
(defn trthumb-bl-place [shape] ; left in tr
  (->> shape
       (rotate (deg2rad   6) [1 0 0])
       (rotate (deg2rad -32) [0 1 0])
...
(defn trthumb-1x-layout [shape]
  (union
   (trthumb-mr-place shape)
   (trthumb-br-place shape)
   (trthumb-tl-place shape)
   (trthumb-bl-place shape)
   ))

(defn trthumb-15x-layout [shape]
  (union
   (trthumb-tr-place shape)))

(def trthumbcaps
  (union
   (trthumb-1x-layout (sa-cap 1))
   (trthumb-15x-layout (rotate (/ π 2) [0 0 1] (sa-cap 1)))))

(def trthumbcaps-fill
  (union
   (trthumb-1x-layout keyhole-fill)
   (trthumb-15x-layout (rotate (/ π 2) [0 0 1] keyhole-fill))))

(def trthumb
  (union
   (trthumb-1x-layout single-plate)
   (trthumb-15x-layout single-plate)))

(def trthumb-post-tr (translate [(- (/ mount-width 2) post-adj)  (- (/ mount-height  2) post-adj) 0] web-post))
(def trthumb-post-tl (translate [(+ (/ mount-width -2) post-adj) (- (/ mount-height  2) post-adj) 0] web-post))
(def trthumb-post-bl (translate [(+ (/ mount-width -2) post-adj) (+ (/ mount-height -2) post-adj) 0] web-post))
(def trthumb-post-br (translate [(- (/ mount-width 2) post-adj)  (+ (/ mount-height -2) post-adj) 0] web-post))

(def trthumb-connectors
  (union
   (triangle-hulls    ; top two
    (trthumb-tl-place web-post-tr)
    (trthumb-tl-place web-post-br)
    (trthumb-tr-place trthumb-post-tl)
    (trthumb-tr-place trthumb-post-bl))
   (triangle-hulls    ; bottom two
    (trthumb-br-place web-post-tr)
    (trthumb-br-place web-post-br)
    (trthumb-mr-place web-post-tl)
    (trthumb-mr-place web-post-bl))
   (triangle-hulls
    (trthumb-mr-place web-post-tr)
    (trthumb-mr-place web-post-br)
    (trthumb-tr-place trthumb-post-br))
   (triangle-hulls    ; between top row and bottom row
    (trthumb-br-place web-post-tl)
    (trthumb-bl-place web-post-bl)
    (trthumb-br-place web-post-tr)
    (trthumb-bl-place web-post-br)
    (trthumb-mr-place web-post-tl)
    (trthumb-tl-place web-post-bl)
    (trthumb-mr-place web-post-tr)
    (trthumb-tl-place web-post-br)
    (trthumb-tr-place web-post-bl)
    (trthumb-mr-place web-post-tr)
    (trthumb-tr-place web-post-br))
   (triangle-hulls    ; top two to the middle two, starting on the left
    (trthumb-tl-place web-post-tl)
    (trthumb-bl-place web-post-tr)
    (trthumb-tl-place web-post-bl)
    (trthumb-bl-place web-post-br)
    (trthumb-mr-place web-post-tr)
    (trthumb-tl-place web-post-bl)
   ...
   (triangle-hulls    ; top two to the main keyboard, starting on the left
    ...
    (trthumb-tr-place trthumb-post-tl)
    (key-place (+ innercol-offset 1) cornerrow web-post-bl)
    (trthumb-tr-place trthumb-post-tr)
    (key-place (+ innercol-offset 1) cornerrow web-post-br)
    (key-place (+ innercol-offset 2) lastrow web-post-tl)
    (key-place (+ innercol-offset 2) lastrow web-post-bl)
    (trthumb-tr-place trthumb-post-tr)
...
(when (= thumb-style "tr")
  (def thumb-type trthumb)
  (def thumb-connector-type trthumb-connectors)
  (def thumbcaps-type trthumbcaps)
  (def thumbcaps-fill-type trthumbcaps-fill))
...
(def tr-thumb-wall
  (union
   ; thumb walls
   (wall-brace trthumb-mr-place  0 -1 web-post-br trthumb-tr-place  0 -1 trthumb-post-br)
   (wall-brace trthumb-mr-place  0 -1 web-post-br trthumb-mr-place  0 -1 web-post-bl)
   (wall-brace trthumb-br-place  0 -1 web-post-br trthumb-br-place  0 -1 web-post-bl)
   (wall-brace trthumb-bl-place  0  1 web-post-tr trthumb-bl-place  0  1 web-post-tl)
   (wall-brace trthumb-br-place -1  0 web-post-tl trthumb-br-place -1  0 web-post-bl)
   (wall-brace trthumb-bl-place -1  0 web-post-tl trthumb-bl-place -1  0 web-post-bl)
   ; trthumb corners
   (wall-brace trthumb-br-place -1  0 web-post-bl trthumb-br-place  0 -1 web-post-bl)
   (wall-brace trthumb-bl-place -1  0 web-post-tl trthumb-bl-place  0  1 web-post-tl)
   ; trthumb tweeners
   (wall-brace trthumb-mr-place  0 -1 web-post-bl trthumb-br-place  0 -1 web-post-br)
   (wall-brace trthumb-bl-place -1  0 web-post-bl trthumb-br-place -1  0 web-post-tl)
   (wall-brace trthumb-tr-place  0 -1 trthumb-post-br (partial key-place (+ innercol-offset 3) lastrow)  0 -1 web-post-bl)
   ; clunky bit on the top left trthumb connection  (normal connectors don't work well)
   (bottom-hull
    (left-key-place (- cornerrow innercol-offset) -1 (translate (wall-locate2 -1 0) web-post))
    (left-key-place (- cornerrow innercol-offset) -1 (translate (wall-locate3 -1 0) web-post))
    (trthumb-bl-place (translate (wall-locate2 -0.3 1) web-post-tr))
    (trthumb-bl-place (translate (wall-locate3 -0.3 1) web-post-tr)))
   (hull
    (left-key-place (- cornerrow innercol-offset) -1 (translate (wall-locate2 -1 0) web-post))
    (left-key-place (- cornerrow innercol-offset) -1 (translate (wall-locate3 -1 0) web-post))
    (trthumb-bl-place (translate (wall-locate2 -0.3 1) web-post-tr))
   ...
   (hull
    (trthumb-bl-place web-post-tr)
    (trthumb-bl-place (translate (wall-locate1 -0.3 1) web-post-tr))
    (trthumb-bl-place (translate (wall-locate2 -0.3 1) web-post-tr))
    (trthumb-bl-place (translate (wall-locate3 -0.3 1) web-post-tr))
    (trthumb-tl-place web-post-tl))
   ; connectors below the inner column to the thumb & second column
   (if inner-column
     (union
      ...
      (hull
       (key-place 0 (dec cornerrow) web-post-bl)
       (key-place 1 cornerrow web-post-bl)
       (trthumb-tl-place trthumb-post-tl))))))
...
(def trrs-notch        (translate (map + usb-holder-position [-10.33 (+ 3.0 notch-offset) 6.6]) (cube 8.4 4.4 19.8)))
...
(when (and pinky-15u (false? extra-row))
    (def screw-offset-tr [1 7 0])
    (def screw-offset-br [2 14 0]))
(when (and pinky-15u (false? extra-row) (> first-15u-row 0))
    (def screw-offset-tr [-4 7 0])
    (def screw-offset-br [2 14 0]))
...
(when (and (= thumb-style "mini") (false? inner-column))
    (def screw-offset-bl [0 4.2 0])
    (def screw-offset-bm [-1 -7 0]))
(when (and (= thumb-style "default") inner-column)
    (def screw-offset-bl [8 -6 0])
    (def screw-offset-bm [8 -1 0]))
(when (and (= thumb-style "default") (false? inner-column))
    (def screw-offset-bl [-11.7 -8 0])
    (def screw-offset-bm [8 -1 0]))
(when (and (= thumb-style "tr") inner-column)
    (def screw-offset-bl [17 8 0])
    (def screw-offset-tm [9.5 -4.5 0])
    (def screw-offset-bm [-1 -10 0]))
(when (and (= thumb-style "tr") (false? inner-column))
    (def screw-offset-bl [0 4.2 0])
    (def screw-offset-bm [-1 -7 0]))

         (defn screw-insert-all-shapes [bottom-radius top-radius height]
  (union (screw-insert 0 0         bottom-radius top-radius height [8 9.5 0])
