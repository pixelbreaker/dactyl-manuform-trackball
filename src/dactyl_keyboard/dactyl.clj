...
(defn column-offset [column] (cond
  (= column 2) [0 2.82 -3.5]
  (>= column 4) [0 -6 3.64]            ; original [0 -5.8 5.64]
  :else [0 0 0]))
...
(defn thumb-tl-place [shape]
  (->> shape
       ...
       (rotate (deg2rad -39) [0 1 0])
       (rotate (deg2rad  23) [0 0 1])
       (translate thumborigin)
       (translate [-36 -15 -11.5])))
...
(def thumb-post-tl (translate [(+ (/ mount-width -2) post-adj -0) (- (/ mount-height  1.15) post-adj) 0] web-post))
...
(def case-walls
  (union
   ...
   (wall-brace thumb-bl-place  0  1 web-post-tr thumb-tl-place  -0.3  1 web-post-tl)
   ...
   (bottom-hull
     ...
     (thumb-tl-place (translate (wall-locate2 -0.3 1) web-post-tl))
     (thumb-tl-place (translate (wall-locate3 -0.3 1) web-post-tl)))
   (hull
     (left-key-place cornerrow -1 (translate (wall-locate2 -1 0) web-post))
     (left-key-place cornerrow -1 (translate (wall-locate3 -1 0) web-post))
     (thumb-tl-place (translate (wall-locate2 -0.3 1) web-post-tl))
   ...
   (hull
     ...
     (thumb-tl-place web-post-tl)
     (thumb-tl-place (translate (wall-locate1 -0.3 1) web-post-tl))
     (thumb-tl-place (translate (wall-locate2 -0.3 1) web-post-tl))
     (thumb-tl-place (translate (wall-locate3 -0.3 1) web-post-tl))
     (thumb-tl-place thumb-post-tl))
  ))
  ...
  (let [shift-right   (= column lastcol)
        ...
        position      (if shift-up     (key-position column row (map + (wall-locate2  0  1) [0 (/ mount-height 2) 0]))
                       (if shift-down  (key-position column row (map - (wall-locate2  0 -1) [0 (/ mount-height 2) 0]))
                        (if shift-left (map + (left-key-position row 0) (wall-locate3 -1 -0.9)) 
...
(defn screw-insert-shape2 [bottom-radius top-radius height] 
   (union (cylinder [bottom-radius top-radius] height)))

(defn screw-insert2 [x y z column row bottom-radius top-radius height] 
  (let [position  (key-position column row (map + (wall-locate2  1  0) [(/ mount-width 2) 0 0]))
        ]
    (->> (screw-insert-shape2 bottom-radius top-radius height)
       (translate [x y z])
       (rotate (deg2rad  -4) [1 0 0])
       (rotate (deg2rad -39) [0 1 0])
...
;         ;               x   y   z                                      height
;         (screw-insert2 -23 -18.5  14 3 0        bottom-radius top-radius 8)
;         (screw-insert2 -50 -12.5  14 3 0        bottom-radius top-radius 8)
;         (screw-insert2 -21  11.5  14 3 0        bottom-radius top-radius 8)
;         (screw-insert2 -52   7  14 3 0          bottom-radius top-radius 8)
;         ))
...
(def wire-posts
  (union
     ;(thumb-tl-place (translate [-5 0 -2] (wire-post  1 0)))
     ;(thumb-tl-place (translate [ 0 0 -2.5] (wire-post -1 6)))
     ;(thumb-tl-place (translate [ 5 0 -2] (wire-post  1 0)))
